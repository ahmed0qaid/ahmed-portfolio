...
const featuredProjects = [
  {
    ...
    descriptionAr:
      "طبقة تحكم أمام n8n وAI Agents تضيف سياسات تنفيذ، موافقات بشرية، MCP execution محكوم، تتبع تكلفة، Retry/Replay، سجل تدقيق، ومرونة أعلى في التعامل مع side effects.",
    descriptionEn:
      "A control plane in front of n8n workflows and AI agents with deterministic policy evaluation, human approval gates, guarded MCP execution, retry/replay, cost accounting, PostgreSQL persistence, and auditability.",
  ...
  },
  {
    ...
    descriptionAr:
      "نواة تنفيذ تعتمد PostgreSQL لوكلاء الذكاء الاصطناعي، تحفظ نقاط التقدم والعمليات ذات الآثار الجانبية وتستعيد العمل بأمان بعد الأعطال، مع worker leases وheartbeats وretry/backoff وحماية التزامن.",
    descriptionEn:
      "A Postgres-first durable execution core for AI agents that checkpoints expensive side effects and safely recovers after crashes, with worker leases, heartbeats, concurrency protection, cancellation, retries, and deterministic tests.",
  ...
  },
  {
    ...
    descriptionAr:
      "بوابة إنفاذ سياسات بين AI agents وخوادم MCP، تتضمن هوية لكل agent، سياسات حسب الدور، approval tokens مرتبطة بالمدخلات، rate limiting، egress allowlists، schema pinning، circuit breakers وسجل تدقيق.",
    descriptionEn:
      "A policy-enforcement gateway between AI agents/MCP clients and real MCP servers with per-agent identity, role-aware policies, HMAC approvals, rate limiting, egress allowlists, schema pinning, circuit breakers, audit logs, tests, and CI.",
  ...
  },
  {
    ...
    descriptionAr:
      "مشروع مراقبة مبني على OpenTelemetry يركز على trajectory الذي يسلكه الوكيل، مع تتبع model/tool/handoff، المحاولات والأخطاء، metadata للاستخدام والتكلفة، استيراد تنفيذات n8n، وتتبع MCP ومقارنة المسارات.",
    descriptionEn:
      "OpenTelemetry-native trajectory tracing for AI agents, covering model/tool/handoff execution, retries and errors, usage and cost metadata, n8n execution imports, MCP tracing, and trajectory diffing.",
  ...
  },
  {
    ...
    descriptionAr:
      "خط ذكاء وأتمتة بدون بنية مدفوعة يجمع بيانات GitHub دوريًا، يحفظ snapshots تاريخية، يحسب trend/opportunity scores، يطابق المهارات في المتصفح، وينشر النتائج تلقائيًا عبر GitHub Actions وGitHub Pages.",
    descriptionEn:
      "A zero-paid-infrastructure intelligence pipeline that discovers rising GitHub repositories and contributor-friendly issues using scheduled collection, historical snapshots, trend/opportunity scoring, skill matching, validation, and automated static deployment.",
  ...
  return (
    <article className="repo-card group flex h-full flex-col rounded-3xl border-cyanBrand/25 p-5 transition hover:-translate-y-1 hover:border-cyanBrand/55 sm:p-6">
      ...
      <p className="mt-3 leading-7 text-slate-300/95 sm:mt-4 sm:leading-8">
        <LocalizedText ar={project.descriptionAr} en={project.descriptionEn} />
      </p>
      ...
      <div className="mt-auto pt-5 sm:pt-6">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-sm font-bold text-cyan-100 transition hover:bg-cyan-300/20"
        >
          <LocalizedText ar="عرض المستودع على GitHub ↗" en="View repository on GitHub ↗" />
        </a>
      </div>
    </article>
  );
  ...
  return (
    <section id="projects" className={isCompact ? "py-9 sm:py-10 lg:py-12" : "py-10 sm:py-12 lg:py-16"}>
      <div className="container-shell">
        ...
        <div className="mt-7 grid gap-4 sm:mt-8 sm:gap-5 lg:mt-10 lg:grid-cols-2 xl:grid-cols-3">
        ...
        {additionalProjects.length ? (
          <div className="mt-10 border-t border-white/[0.07] pt-8 sm:mt-12 sm:pt-10">
            <div className="mb-5 sm:mb-6">
              <p className="text-sm font-bold text-cyanBrand">
                  ...
                  <article key={`${project.name}-${index}`} className="repo-card group flex h-full flex-col rounded-3xl p-5 transition hover:-translate-y-1 hover:border-violetBrand/50 sm:p-6">
                    ...
                    {isAttendanceSuite ? (
                      <div className="mt-5 grid grid-cols-2 gap-2">
                        {attendanceRepositories.map((repo) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="flex min-h-10 items-center justify-between gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-2.5 py-2 text-[11px] font-bold leading-4 text-slate-100 transition hover:border-cyan-300/40 hover:bg-cyan-300/10 sm:text-xs"
                          >
                      ...
                      <div className="mt-5 flex flex-wrap gap-2.5 sm:mt-6 sm:gap-3">
                        <a href="https://github.com/ahmed0qaid/edge-cloud-collaborative-scheduling" target="_blank" rel="noreferrer" className="rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-sm font-bold text-cyan-100 transition hover:bg-cyan-300/20">
                          <LocalizedText ar="GitHub Repository" en="GitHub Repository" />
                        </a>
                        <a href="https://codeforces.com/contest/2251/problem/A" target="_blank" rel="noreferrer" className="rounded-xl border border-violet-300/30 bg-violet-300/10 px-4 py-2 text-sm font-bold text-violet-100 transition hover:bg-violet-300/20">
